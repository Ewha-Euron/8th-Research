...
class ResConvGroupNorm(nn.Module):
    """
    Residual + Convolution + BatchNorm 블록.

    입력 채널 수 → 출력 채널 수로 변환하고,
    잔차(residual connection)를 추가.
    """
...
class UNet(nn.Module):
    ...
    def __init__(self, dim_emb: int = 1024):
        ...
        # Now input is 64x7x7 Bottleneck
        ...
        # Now input is 64x14x14 Upsampling 1
        new_ch = ch[2] + ch[1]
        ...
        # Now input is 16x28x28 Upsampling 2
        new_ch = ch[3] + ch[0]
...
class ConditionalUNet(UNet):
    """
    Label Embedding을 추가한 Conditional U-Net.
    Class Conditioning을 통해 특정 label에 맞는 이미지 생성 가능.
    """
    def __init__(self, dim_emb: int = 1024):
