...
class DiffusionProcess:
    ...
    """
    """
        DDPM 논문(https://arxiv.org/pdf/2006.11239.pdf)을 따라
        Forward 및 Inverse Diffusion 과정을 구현한 클래스입니다.

        Attributes:
            variance_schedule (torch.Tensor): 각 timestep마다의 분산 값 (β_t)
            alpha (torch.Tensor): α_t = 1 - β_t
            alpha_bar (torch.Tensor): ᾱ_t = 누적 곱 (cumulative product of α_t)
    """
    ...
    def __init__(self, variance_schedule: List[float] = None) -> None:
        ...
        """
        """
            DiffusionProcess 클래스 생성자.
            Args:
                variance_schedule (list): timestep별 분산(β_t) 리스트.
                기본값은 1e-4에서 0.01까지 선형 증가하는 1000개 값.
        """
    ...
    def forward(self, x_0: Tensor, time_step: Tensor, noise: Tensor) -> Tensor:
        ...
        """
        """
            주어진 timestep까지 original image (x_0)에 forward diffusion 적용.
            수식:
                x_t = sqrt(ᾱ_t) * x_0 + sqrt(1-ᾱ_t) * noise

            Args:
                x_0 (Tensor): 원본 이미지
                time_step (Tensor): 적용할 timestep (batch 크기와 일치)
                noise (Tensor): 표준 정규분포에서 샘플링한 노이즈

            Returns:
                Tensor: Diffused image x_t
        """
    ...
    def inverse(self, xt: Tensor, et: Tensor, t: int) -> Tensor:
        ...
        """
        """
            Reverse Diffusion (Sampling) 과정 구현 (DDPM 방식).

            수식:
                μ_t = (1/√α_t) * (x_t - (1-α_t)/√(1-ᾱ_t) * e_theta(x_t, t))
                x_{t-1} ~ N(μ_t, β_t * I)
            Args:
                xt (Tensor): timestep t의 noisy image
                et (Tensor): 예측된 노이즈 ε
                t (int): 현재 timestep
    ...
    def inverse_DDIM(self, xt: Tensor, et: Tensor, t: int) -> Tensor:
        ...
        """
        """
        Deterministic DDIM Sampling 구현.

        std_dev = 0이므로 stochastic noise 없이 샘플링.
        훨씬 빠르게 샘플링 가능 (10~100배).
